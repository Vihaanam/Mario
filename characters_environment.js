...
function manualControl(character){
  ...
  if(character.live){
    if(noseX < 250){
      ...
      character.changeAnimation('move');
      character.mirrorX(-1);
      gameConfig.moveSpeed=(300-noseX)/15;
    }
    ...
    if(noseX > 350){
      ...
      character.changeAnimation('move');
      character.mirrorX(1);
      gameConfig.moveSpeed=(noseX-300)/15;
    }

    if(noseX < 350 &&!noseX > 250 &&!noseY < 148){ 
...
function jumping(character){
	if( (noseY < 147 &&character.live) || (touchIsDown&&character.live) ){
...
function moveEnvironment(character){
  var environmentScrollingSpeed=1.5; 
